            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  {getValidationBadge(profile.validation_status)}
                </div>
                ...
                <div className="flex flex-wrap gap-4 text-sm text-slate-600">
                  {profile.connections_count > 0 && (
                    <div>
                      <span className="font-medium">{profile.connections_count}</span> connections
                    </div>
                  )}
                  {profile.experience && profile.experience.length > 0 && (
                    <div>
                      <span className="font-medium">{profile.experience.length}</span> experiences
                    </div>
                  )}
                ...
                {profile.validation_errors && profile.validation_errors.length > 0 && (
                  <div className="mt-3 p-3 bg-red-50 rounded-lg">
                    ...
                    <ul className="text-sm text-red-700 space-y-1">
                      {profile.validation_errors.slice(0, 3).map((error, idx) => (
